...
const testExeEnv = "GAME_TEST_STRING"
...
func TestTransitiveHashFast(t *testing.T) {
	...
	// return "meow", because we're only hashing the top level gamefiles, not
	// dependencies.
	if actual := stdout.String(); !strings.Contains(actual, expected) {
		t.Fatalf("expected %q but got %q", expected, actual)
	}
...
func TestListGamefilesIgnoresGOOS(t *testing.T) {
	buf := &bytes.Buffer{}
	...
	files, err := Gamefiles("testdata/goos_gamefiles", "", "", "go", buf, false)
	if err != nil {
		t.Errorf("error from gamefile list: %v: %s", err, buf)
	}
	var expected []string
...
func TestListGamefilesIgnoresRespectsGOOSArg(t *testing.T) {
	buf := &bytes.Buffer{}
	var goos string
	if runtime.GOOS == "windows" {
		goos = "linux"
	...
	files, err := Gamefiles("testdata/goos_gamefiles", goos, "", "go", buf, false)
	if err != nil {
		t.Errorf("error from gamefile list: %v: %s", err, buf)
	}
	var expected []string
...
func TestListGamefilesLib(t *testing.T) {
	buf := &bytes.Buffer{}
	files, err := Gamefiles("testdata/mixed_lib_files", "", "", "go", buf, false)
	if err != nil {
		t.Errorf("error from gamefile list: %v: %s", err, buf)
	}
...
func TestMixedGameImports(t *testing.T) {
	stderr := &bytes.Buffer{}
	stdout := &bytes.Buffer{}
	inv := Invocation{
...
func TestOnlyStdLib(t *testing.T) {
	...
	for _, s := range f.Imports {
		...
		if !filepath.HasPrefix(pkg.Dir, build.Default.GOROOT) &&
			pkg.ImportPath != "github.com/ridge/game/mg" &&
			pkg.ImportPath != "github.com/ridge/game/toplevel" {
...
var wrongDepRx = regexp.MustCompile("Invalid type for a task function.*@ main.FooBar .*gamefile.go")
